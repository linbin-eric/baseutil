package xin.nb1.baseutil.bytecode.annotation;

import xin.nb1.baseutil.bytecode.ClassFile;
import xin.nb1.baseutil.bytecode.ClassFileParser;
import xin.nb1.baseutil.bytecode.structure.AnnotationInfo;
import xin.nb1.baseutil.bytecode.structure.Attribute.AttributeInfo;
import xin.nb1.baseutil.bytecode.structure.Attribute.RuntimeVisibleAnnotationsAttriInfo;
import xin.nb1.baseutil.bytecode.structure.MethodInfo;
import xin.nb1.baseutil.bytecode.support.OverridesAttribute;
...
public class SupportOverrideAttributeAnnotationMetadata extends AbstractAnnotationMetadata
{
    ...
    private SupportOverrideAttributeAnnotationMetadata(String resourceName, Map<String, ValuePair> attributes, ClassLoader loader)
    {
        ...
        Arrays.stream(classFile.getMethodInfos()).flatMap(methodInfo -> Arrays.stream(methodInfo.getAttributeInfos()).map(attributeInfo -> new StreamData(methodInfo, attributeInfo))).filter(data -> data.attributeInfo instanceof RuntimeVisibleAnnotationsAttriInfo).flatMap(data -> Arrays.stream(((RuntimeVisibleAnnotationsAttriInfo) data.attributeInfo).getAnnotations()).map(annotation -> new StreamData(data.methodInfo, annotation))).filter(data -> data.annotationInfo.getType().equals(name)).forEach(data -> {
